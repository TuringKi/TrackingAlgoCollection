...
obspathFormat = 'results/%s.txt';
 TEST_SETS = {'woman_sequence' 'Occlusion1' 'Occlusion2' 'Singer1'...
     'davidin300' 'bike' 'Face' 'board' 'Stone' 'Caviar1' 'Caviar2' 'Caviar3' 'Car4'};
  gt_path = '';
  obs_path = '';
 ...
 for i = 1:length(TEST_SETS)
      ...
      struckCenterAll = cell(size(gtCenterAll));
     
      tCornersAll = cell(size(gtCornersAll));
    
      struckCornersAll = cell(size(gtCornersAll));
      ...
      for j = 1:length(obs_data)
       ...
       x2 = [cx + w/2,cy - h/2]';
        ...
        struckCornersAll{1,j} = tCornersAll{1,j};
        
        all_time = all_time + t;
       struckCenterAll{1,j} = [cx,cy]';
      
      end
      fps = length(frameIndex)/all_time;
     save([ 'outputdata/' TEST_SETS{i} '_struck_rs.mat'], 'struckCenterAll', 'struckCornersAll', 'fps')
