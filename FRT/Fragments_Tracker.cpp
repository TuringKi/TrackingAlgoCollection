...
{
	CvPoint tl;
	tl.y = MIN(tl_y , I->height - 6);
	tl.x = MIN(tl_x , I->width - 6);
	
	CvPoint br;
	br.y =MIN( tl_y + height - 1,I->height - 6);
	br.x = MIN(tl_x + width - 1,I->width - 6);
	cout << "x:" << br.x << "y" << br.y << endl;
	cvRectangle(I, tl, br, CV_RGB(255,0,0), 1 );
}
											   ...
											   vector< vector<double>* >& patch_histograms)
{
	...
	for ( it2 = patches.begin() ; it2 != patches.end() ; it2++)
	{
		...
		ctr = ctr + 1;
		if (ctr < 7)
		Draw_Rectangle(p_cy - (*it2)->h, p_cx - (*it2)->w, p_cy + (*it2)->h, p_cx + (*it2)->w, cur_I);
	}
